...
int main(int argc, char* argv[])
{
	...
	if (argc != 3)
	{
		printf("static_dipole [charge density in cube file 1] [2]\n");
	...
	double* dipoles1 = calStaticDipole(wf1, cInfo);
	double* dipoles2 = calStaticDipole(wf2, cInfo);
	double stark_dipole_X = dipoles2[0] - dipoles1[0];
	double stark_dipole_Y = dipoles2[1] - dipoles1[1];
	double stark_dipole_Z = dipoles2[2] - dipoles1[2];

	to_mXmeVpMV = 5.2918/100;

	printf("1: %f 2: %f\n", dipoles1[0]*to_mXmeVpMV, dipoles2[0]*to_mXmeVpMV);
	printf("1: %f 2: %f\n", dipoles1[1]*to_mXmeVpMV, dipoles2[1]*to_mXmeVpMV);
	printf("1: %f 2: %f\n", dipoles1[2]*to_mXmeVpMV, dipoles2[2]*to_mXmeVpMV);
	printf("--dipole matrix elements--\n");
	printf("X matrix : %.5lf m*meV/MV\n", stark_dipole_X*to_mXmeVpMV);
	printf("Y matrix : %.5lf m*meV/MV\n", stark_dipole_Y*to_mXmeVpMV);
	printf("Z matrix : %.5lf m*meV/MV\n", stark_dipole_Z*to_mXmeVpMV);
...
double*** readCubeWf(char* filein, pCubeInfo cInfo)
{
	...
	for (int i = 0; i < Nx; i++)
	{
		for (int j = 0; j < Ny; j++)
		{
			...
			{
				fscanf(fCube, "%s", temp);
				wf[i][j][k] = fabs(sciNotToDouble(temp));
			}
		}
	}
