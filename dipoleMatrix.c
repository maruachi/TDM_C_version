...
double*			calDipoleMatrix(double*** wf1, double*** wf2, pCubeInfo cInfo);
double			calOverlap(double*** wf1, double*** wf2, pCubeInfo cInfo);
...
int main(int argc, char* argv[])
{
	...
	if (argc != 3)
	{
		printf("dipoleMatrix [cube file 1] [cube file 2]\n");
		printf("this program calculate the dipole matrix <1|2>\n");
	}
	...
	wf2 = readCubeWf(fileWf2, cInfo);
	
	double normWf1 = nomalization(wf1, cInfo);
	double normWf2 = nomalization(wf2, cInfo);

	double* dipoles = calDipoleMatrix(wf1, wf2, cInfo);
	double overlap = calOverlap(wf1, wf2, cInfo);

	printf("--confirm wavefunction--\n");
	printf("normalzation of wf1, wf2 = %4.3lf, %4.3lf\n", normWf1, normWf2);
	printf("overlap of between wf1 and wf2 = %4.3lf\n", overlap);
...
double nomalization(double*** wf, pCubeInfo cInfo)
{
	...
	for (int i = 0; i < Nx; i++)
	{
		for (int j = 0; j < Ny; j++)
		{
			for (int k = 0; k < Nz; k++)
			{
				infDensity = pow(wf[i][j][k], 2) * infVol; 
				totDensity += infDensity;
			}
		}
	}
	...
	for (int i = 0; i < Nx; i++)
	{
		for (int j = 0; j < Ny; j++)
		{
			for (int k = 0; k < Nz; k++)
			{
				...
				Xdipole += wf1[i][j][k] * position[0] * wf2[i][j][k] * infVol;
				Ydipole += wf1[i][j][k] * position[1] * wf2[i][j][k] * infVol;
				Zdipole += wf1[i][j][k] * position[2] * wf2[i][j][k] * infVol;
			}
		}
	}
...
double calOverlap(double*** wf1, double*** wf2, pCubeInfo cInfo)
{
	...
	for (int i = 0; i < Nx; i++)
	{
		for (int j = 0; j < Ny; j++)
		{
			for (int k = 0; k < Nz; k++)
			{
				overlap += wf1[i][j][k] * wf2[i][j][k] * infVol;
			}
		}
	}
